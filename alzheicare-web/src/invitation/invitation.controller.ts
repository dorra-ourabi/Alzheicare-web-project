import { Body, Controller, Get, Post, Patch, UseGuards, Query, Param, ParseIntPipe } from '@nestjs/common';
...
export class InvitationController {
  ...
  @Patch(':id/respond')
  respond(@CurrentUser() user: any, @Param('id', ParseIntPipe) id: number, @Body() dto: RespondInvitationDto) {
  ...
  respondViaToken(@CurrentUser() user: any, @Body('token') token: string, @Body('action') action: RespondStatus) {
    return this.invitationService.respondViaToken(token, user.sub, action);
  }
}
