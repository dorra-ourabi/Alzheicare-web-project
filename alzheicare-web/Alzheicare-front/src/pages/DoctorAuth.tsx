  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await apiRequest<AuthTokens>(
        tab === 'login' ? '/auth/login' : '/auth/register/doctor',
        {
          method: 'POST',
          body:
            tab === 'login'
  ...
  const handleGoogleSuccess = async (credential: string | undefined) => {
    ...
    try {
      const response = await apiRequest<AuthTokens>('/auth/google-login', {
        method: 'POST',
        body: JSON.stringify({ idToken: credential, role: 'Doctor' }),
      })
    ...
    <div className="min-h-screen flex" style={{ background: '#f8faff' }}>
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center gap-6">
          ...
          <p className="text-white/60 text-sm tracking-widest uppercase text-center">Medical Doctor Portal</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        ...
        >
          ...
          <p className="text-sm text-gray-400 mb-6">Sign in to your doctor account or create one</p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            {tab === 'register' && (
              <>
                <Field label="Username" value={form.username} onChange={(value) => update('username', value)} placeholder="drjones" />
                <div className="grid grid-cols-2 gap-3">
                  <Field label="First Name" value={form.firstName} onChange={(value) => update('firstName', value)} placeholder="John" />
                  <Field label="Second Name" value={form.secondName} onChange={(value) => update('secondName', value)} placeholder="Jones" />
                </div>
                <Field label="Email" value={form.email} onChange={(value) => update('email', value)} placeholder="doctor@example.com" type="email" />
                <Field label="Medical License Number" value={form.licenceNumber} onChange={(value) => update('licenceNumber', value)} placeholder="ML-2024-00123" />
                <Field label="Specialization" value={form.specialization} onChange={(value) => update('specialization', value)} placeholder="Geriatrics" />
              </>
            )}
