...
const displayStageName = (stage: string) => stageDisplayName[stage] ?? stage;
...
export default function MRIClassifier() {
  ...
  const classify = async () => {
    ...
    try {
      const data = await predictMri(file, accessToken);
      setResult(data);
    } catch (err) {
  ...
  const tone = result
    ? (stageTone[result.predicted_stage as keyof typeof stageTone] ??
      defaultTone)
    : defaultTone;
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400">
            Upload a brain scan to detect Alzheimer's stage
          </p>
        </div>
      </div>
          ...
          {result ? (
            <div className={`rounded-2xl border-2 p-5 ${tone.bg}`}>
              <div className="flex items-center justify-between gap-4 mb-4">
                ...
                <span className={`text-2xl font-bold text-right ${tone.color}`}>
                  {displayStageName(result.predicted_stage)}
                </span>
              </div>
                  ...
                  ([label, probability]) => (
                    <div key={label}>
                      <div className="flex justify-between gap-3 text-xs text-gray-500 mb-1">
                        <span>{displayStageName(label)}</span>
                        <span className="font-semibold">
            ...
            <div className="flex flex-col items-center justify-center h-full text-center gap-3 py-10">
              ...
              <p className="text-sm text-gray-400">
                Upload a scan and click Classify to see results
              </p>
            </div>
          )}
        </div>
