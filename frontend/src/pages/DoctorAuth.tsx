...
export default function DoctorAuth() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (tab === 'register') {
        ...
        const response = await registerUser(username, email, password, firstName || fullName, lastName || '')
        // After registration we expect the user to verify email — show a notice instead of auto-login
        navigate(`/register?role=doctor&registered=true`)
      } else {
  ...
  // If ?tab=register provided in url, open register tab
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">{tab === 'login' ? 'Username' : 'Username'}</label>
              <input
                type="text"
                placeholder={tab === 'login' ? 'username' : 'Choose a username'}
                value={username}
            ...
            <button
              ...
              style={{
                background: 'linear-gradient(135deg, #1a6fb5 0%, #0d2f7a 100%)',
          ...
          <div className="mt-6 flex flex-col gap-2">
            ...
            <button
              ...
              className="w-full text-center text-xs text-blue-600 hover:text-blue-800 transition"
            >
